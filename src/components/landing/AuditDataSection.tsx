      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 sm:max-w-xl md:max-w-7xl lg:max-w-6xl mx-auto transition-all duration-300">
        {stats.map((stat) => (
          <div key={stat.label} className="text-center">
            <p className="text-edgi-green text-4xl md:text-5xl font-bold mb-1 sm:mb-3 transition-all duration-300">
              {stat.number}
            </p>
            <p className="text-edgi-gray text-sm md:text-base leading-relaxed">
              {stat.label}
            </p>
          </div>
        ))}
